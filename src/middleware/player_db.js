...
async function coordinateDatabaseOps(req, res, next) {
	try {
		if (!req.body) {
			const clientError = new CustomError('No body in request', 400);
			return res.status(clientError.status).json(clientError.message);
		}
		const { gameCode, dirKey } = req.body;
		if (!gameCode) {
			const clientError = new CustomError('No Game Code in request', 401);
			return res.status(clientError.status).json(clientError.message);
		}
		if (!dirKey) {
			const clientError = new CustomError('No Director Key in request', 401);
			return res.status(clientError.status).json(clientError.message);
		}
...
async function coordinateBwFromOps(req, res, next) {
	try {
		...
		if (!formData) {
			const clientError = new CustomError('No form data in request', 400);
			return res.status(clientError.status).json(clientError.message);
		}
...
async function handleDatabaseImport(req, res, next) {
	try {
		...
		if (!gameCode || !dirKey || !importData || !meta) {
			if (!importData) {
				const clientError = new CustomError('No import data', 400);
				return res.status(clientError.status).json(clientError.message);
			}
			if (!meta) {
				const clientError = new CustomError('No import meta data', 400);
				return res.status(clientError.status).json(clientError.message);
			} else {
				const clientError = new CustomError('No credentials in request', 401);
				return res.status(clientError.status).json(clientError.message);
			}
		}
